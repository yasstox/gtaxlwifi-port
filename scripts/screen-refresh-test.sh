...
if confirm "Write 0 to fb0/blank?"; then
  printf '%s\n' "${GTAXL_SSH_PASSWORD:-}" | "${ssh_base[@]}" "sudo -S -p '' sh -c 'echo 0 > /sys/class/graphics/fb0/blank'" || true
  echo "Observe the screen."
fi

if confirm "Force a framebuffer pan/refresh (echo 0,0 > fb0/pan)?"; then
  printf '%s\n' "${GTAXL_SSH_PASSWORD:-}" | "${ssh_base[@]}" "sudo -S -p '' sh -c 'echo 0,0 > /sys/class/graphics/fb0/pan'" || true
  echo "Observe the screen."
fi
...
if confirm "Start msm-fb-refresher --loop temporarily if available?"; then
  printf '%s\n' "${GTAXL_SSH_PASSWORD:-}" | "${ssh_base[@]}" "sudo -S -p '' sh -c 'command -v msm-fb-refresher >/dev/null && nohup msm-fb-refresher --loop >/tmp/msm-fb-refresher.log 2>&1 &'" || true
